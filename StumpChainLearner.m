classdef StumpChainLearner < Learner
    ...
    methods
        function [ self ] = StumpChainLearner(X, Y, opts)
            % Simple constructor for boosted stump chains, which computes a
            % constant value to initialize the set of stump chains.
            if ~exist('opts','var')
                opts = struct();
            end
            if ~isfield(opts,'nu')
        ...
        function [ L ] = extend_mdm(self, X, Y, keep_it)
            ...
            for r_num=1:self.update_rounds,
                ...
                for u_num=1:numel(stump_idx),
                    ...
                    leaf_idx = StumpChainLearner.get_leaf_idx(X, other_stumps);
            ...
            leaf_idx = StumpChainLearner.get_leaf_idx(X, stump_chain.stumps);
            for l=1:max(leaf_idx),
                idx = find(leaf_idx == l);
                if (numel(idx) > 0)
                    w = self.find_step(...
                        F, ones(size(F)), @( f ) self.loss_func(f, Y, idx));
                else
                    w = 0;
                end
        ...
        function [ F ] = evaluate(self, X, idx)
            ...
            for s_num=1:numel(idx),
                ...
                l_idx = StumpChainLearner.get_leaf_idx(X, stump_chain.stumps);
