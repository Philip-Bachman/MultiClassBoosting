function [ L dLdF ] = loss_huberhinge(F, Y, idx, loss_vec)
% Compute the objective function value and functional gradient for Huberized
% hinge loss, given the values in F and weighted target classes in Y.
%
% Parameters:
...
if (nargout > 1)
    ...
    dLdF = zeros(size(F));
    dLdF(fy < -1) = -4;
    dLdF((fy >= -1) & (fy <= 1)) = -((1 - fy((fy >= -1) & (fy <= 1))) .* 2);
    dLdF = dLdF .* Y;
end
