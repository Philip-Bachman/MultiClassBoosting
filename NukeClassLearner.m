classdef NukeClassLearner < Learner
    ...
    % penalty on diffusion of the observations about their class centers/codes.
    %
    ...
    %   opts.lam_nuc: This weight controls the strength of nuclear norm
    ...
    methods
        ...
        function [ self ] = NukeClassLearner(X, Y, opts)
            ...
            Cy = unique(Y);
            self.c_labels = Cy(:);
            self.c_codes = NukeClassLearner.init_codes(...
        ...
        function [ L dLdF ] = compute_loss_grad(self, F, Y, loss_func, idx)
            ...
            L_dif = (self.lam_dif * sum(Dp)) / obs_count;
            L = L_class + L_dif;
            ...
            if (nargout > 1)  
                ...
                dLdF = (2 * (dGrads - (dLc * self.c_codes))) ./ obs_count;
            end
            return
        end
        ...
        function [ L dLdC ] = code_loss_grad(self, codes, F, Y, loss_func)
            ...
            sv_idx = find(S);
            sv = S(sv_idx);
            sv_sabs = sqrt(sv.^2 + 1e-4);
            % Loss is sum of classification, diffusion, and nuclear-norm losses
            L_class = sum(sum(Lc)) / obs_count;
            L_dif = (self.lam_dif * sum(Dp)) / obs_count;
            L_nuc = (self.lam_nuc * sum(sv_sabs)) / numel(sv_sabs);
            L = L_class + L_dif + L_nuc;
            % Compute gradients if they are requested
            if (nargout > 1)  
                % dLc is gradient of loss with respect to the differences
                % between distances. dLc(i,j) is the derivative of the loss
